 ...
 *
 * XML defines a mechanism for inserting instructions (like PHP) into a document. These are called "Processor
 * Instructions." The HTML5 parser provides an opportunity to handle these processor instructions during the
 * tree-building phase (before the DOM is constructed), which makes it possible to alter the document as it is being
 * created.
 *
...
interface InstructionProcessorInterface
{
     ...
     *
     * @return \DOMElement The element that should be considered "Current". This may just be the element passed in, but
     *                     if the processor added more elements, it may choose to reset the current element to one of
     *                     the elements it created. (When in doubt, return the element passed in.)
     */
